...
module.exports = class NamespaceSocketHandler {
    ...
    uploadFiles(socket) {
        socket.on("upload", async ({ file, filename, roomName, endpoint, userId }, callback) => {
            const ext = path.extname(filename);
            let newFileName = `public/uploads/sockets/${String(Date.now() + ext)}`
            fs.writeFile(newFileName, file, (err) => {
                callback({ message: err ? "failure" : "success" });
            });
            newFileName = newFileName.slice(7)
            await conversationModel.updateOne({ endpoint, "rooms.name": roomName }, {
                $push: {
                    "rooms.$.messages": {
                        image: newFileName,
